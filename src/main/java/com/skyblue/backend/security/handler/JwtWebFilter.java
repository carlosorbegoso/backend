...
//                .doOnNext(i->log.info("{}",i))
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (auth == null) {
            ...
            return this.writeErrorMessage(response, HttpStatus.NOT_ACCEPTABLE, "token without" + jwtSigner.getTokenPrefix() + "start");
        }
...
//                                    .doOnNext(info->log.info("{}", info))
                                    .flatMap(authentication -> {
                                        if (authentication.getPrincipal() != null) {
                                            ...
                                            return this.writeErrorMessage(response, HttpStatus.INTERNAL_SERVER_ERROR, "Access parsing is successful, but do not have permission");
                                        }
                                    });
                        } catch (Exception e) {
                        ...
                        return this.writeErrorMessage(response, HttpStatus.UNAUTHORIZED, "illegal token，The token has not been released，or has been deleted");
                    }
                });
    }
}